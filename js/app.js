...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <p>Rating: <span class="fw-bold text-danger">${product.rating.rate}</span> (${product.rating.count})</p>
      <h5>Price: $ ${product.price}</h5>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button onclick="productDetails(${product.id})" id="addToCart-btn" class="buy-now btn btn-danger">details</button>
      </div>
      `;
...
const searchResult = () => {
  ...
  if (inputText) {
    ...
    for (const product of products) {
      const productTitle = product.children[0].children[1].innerText.toLowerCase();
      if (productTitle.includes(inputText)) {
